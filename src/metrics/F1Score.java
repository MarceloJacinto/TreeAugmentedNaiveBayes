...
import metrics.perclass.F1PerClass;
 ...
 * using the F1Score formula. This class receives a ConfusionMatrix object that must be set (in it's constructor).
 ...
 * @since 2020-05-03
 */
public class F1Score<T extends Comparable<T>> extends AbstractMultipleCategoryScore<T> {

    /**
     * Constructor for the F1Score. After receiving a ConfusionMatrix object it calculates the score for each class
     * @see F1PerClass
     * @see ConcreteConfusionMatrix
     * @see ConfusionMatrix
     * @param confusionMatrix       A ConfusionMatrix object that must be set
     */
    public F1Score(ConfusionMatrix<T> confusionMatrix) {
        super(confusionMatrix, new F1PerClass(confusionMatrix.getConfusionMatrix()));
    }
