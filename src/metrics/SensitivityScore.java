...
import metrics.perclass.SensitivityPerClass;
 ...
 * using the Sensitivity Score formula. This class receives a ConfusionMatrix object that must be set (in it's constructor).
 ...
 * @since 2020-05-03
 */
public class SensitivityScore<T extends Comparable<T>> extends AbstractMultipleCategoryScore<T> {

    /**
     * Constructor for the SensitivityScore. After receiving a ConfusionMatrix object it calculates the score for each class
     * @see SensitivityPerClass
     * @see ConcreteConfusionMatrix
     * @see ConfusionMatrix
     * @param confusionMatrix       A ConfusionMatrix object that must be set
     */
    public SensitivityScore(ConfusionMatrix<T> confusionMatrix) {
        super(confusionMatrix, new SensitivityPerClass(confusionMatrix.getConfusionMatrix()));
    }
